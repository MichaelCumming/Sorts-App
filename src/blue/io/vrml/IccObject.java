...
class IccObject {
    ...
    private static String translation(Vector position) {
        ...
        return position.w().scale(position.x()).doubleValue() + " " + position.w().scale(position.y()).doubleValue() + " " +
    ...
    void label(String label) {
        ...
        if (z == -2) z = 0;
        this.position = base.add(
            new Vector(new Coord(10 * x), new Coord(-5 * this.count - 1), new Coord(10 * z), new Coord(5)));
    ...
    void satellite(String label) {
        ...
        this.position = base.add(new Vector(new Coord(x), Coord.ZERO, new Coord(z), new Coord(2)));
        this.offset = "0 0 0";
    ...
    String controlPanel() {
        ...
        if (this.group.associate() != null) {
            ...
            if ((assoc.group.associate() == null) && (assoc.count == 1))
                buffer.append("       whichChoice 0 }\n      DEF ");
            else
                buffer.append("       whichChoice -1 }\n      DEF ");
        } else
    ...
    String controlRoute() {
        ...
        if (this.count > 1)
            buffer.append("ROUTE ").append(this.name("LARROW_TOUCH")).append(".isActive TO ").append(this.name("SELECT",
                this.count - 1)).append(".set\n");
        if (this.count < this.group.maxcount())
            buffer.append("ROUTE ").append(this.name("RARROW_TOUCH")).append(".isActive TO ").append(this.name("SELECT",
                this.count + 1)).append(".set\n");
        if (this.attribute != null)
            buffer.append("ROUTE ").append(this.name("DARROW_TOUCH")).append(".isActive TO ").append(this.attribute.instance().name("SELECT",
                1)).append(".set\n");
        // select
        ...
        if (this.attribute != null)
            for (int n = 1; n <= this.attribute.maxcount(); n++) {
                buffer.append(select).append(".settrue TO ").append(this.attribute.instance().name("DESELECT",
                    n)).append(".set\n");
                buffer.append(select).append(".on TO ").append(this.attribute.instance().name("ITEM",
                    n)).append(".whichChoice\n");
            }
        // deselect
        ...
        if (this.attribute != null)
            for (int n = 1; n <= this.attribute.maxcount(); n++) {
                buffer.append(deselect).append(".settrue TO ").append(this.attribute.instance().name("DESELECT",
                    n)).append(".set\n");
                buffer.append(deselect).append(".off TO ").append(this.attribute.instance().name("ITEM",
                    n)).append(".whichChoice\n");
            }
...
*/
        ...
        if (this.type == JUMP) {
            ...
            buffer.append("ROUTE ").append(this.name("JUMP_TOUCH")).append(".isActive TO ").append(this.name("HIDE",
                1)).append(".set\n");
            if (this.target != null) {
