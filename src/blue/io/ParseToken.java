...
class ParseToken implements Parser {
    // representation
    ...
    int parse() {
        ...
        if ((this.reader.index == this.reader.string.length()) && (this.reader.readLine() == null)) return this.reader.index;
        if (this.advance())
    ...
    private boolean advance() {
        ...
        while (((this.value == ' ') || (this.value == '\t')) && (this.index < this.text.length()))
            this.value = this.text.charAt(this.index++);
        ...
        if (((this.value >= 'a') && (this.value <= 'z')) ||
            ((this.value >= 'A') && (this.value <= 'Z')) || (this.value == '_'))
    ...
    private int span(int lead) {
        ...
        switch (this.value) {
            case STRING:
                ...
                } while ((lead > 0) && (this.text.charAt(index - 2) == '\\'));
                if (lead < 1) {
            ...
            case IDENTIFIER:
                ...
                return lead - 1;
            case NUMBER:
                do { current = this.text.charAt(lead++); }
                while ((lead < this.text.length()) && (current >= '0') && (current <= '9'));
                if ((current == '.') && (lead < this.text.length())) {
                    do { current = this.text.charAt(lead++); }
                    while ((lead < this.text.length()) && (current >= '0') && (current <= '9'));
                    if (((current == 'e') || (current == 'E')) && (lead < this.text.length())) {
                        current = this.text.charAt(lead++);
                        if (((current == '+') || (current == '-')) && (lead < this.text.length())) {
                            do { current = this.text.charAt(lead++); }
                            while ((lead < this.text.length()) && (current >= '0') && (current <= '9'));
                        }
                    }
